<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            ...
            @auth
                <div class="sm:flex sm:items-center sm:ms-6 gap-3">
                    <div class="font-medium text-sm text-gray-700 dark:text-gray-200">{{ Auth::user()->name }}</div>

                    <!-- Logout visível no header -->
    ...
    {{-- @if(Auth::check())
        ...
        <div class="text-sm text-green-600">Auth::check() = true — {{ Auth::user()->name }}</div>
    @else
        <!-- DEBUG: usuário NÃO autenticado -->
        <div class="text-sm text-red-600">Auth::check() = false</div>
    @endif --}}
</nav>
